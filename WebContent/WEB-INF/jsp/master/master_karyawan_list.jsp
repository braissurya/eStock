		...
		<div class="inner">
       		<table class="table">
       			<tr>
       				...
       				<th>
       					<script>groupList('nama', 'form', '${page}', 'Nama', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       					<script>groupList('jeniskry', 'form', '${page}', 'Posisi', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       					<script>groupList('tgl_masuk', 'form', '${page}', 'Tgl Masuk', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       					<script>groupList('gaji', 'form', '${page}', 'Gaji Pokok', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       					<script>groupList('makan', 'form', '${page}', 'Uang Makan', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       					<script>groupList('transport', 'form', '${page}', 'Uang Transport', '${sort}', '${sort_type}');</script>
       				</th>
					<th>
       			...
       			<c:forEach items="${listPaging}" var="p" varStatus="s">
       				<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
       					...
       					<td>
       						<fmt:formatDate value="${p.tgl_masuk}" pattern="dd-MM-yyyy"/>
       					</td>
       					<td>
       						<fmt:formatNumber type="currency" currencySymbol="Rp. "  value="${p.gaji}" maxFractionDigits="2" minFractionDigits="0" ></fmt:formatNumber> 
       					</td>
       					<td>
       						<fmt:formatNumber type="currency" currencySymbol="Rp. "  value="${p.makan}" maxFractionDigits="2" minFractionDigits="0" ></fmt:formatNumber> 
       					</td>
       					<td>
       						<fmt:formatNumber type="currency" currencySymbol="Rp. "  value="${p.transport}" maxFractionDigits="2" minFractionDigits="0" ></fmt:formatNumber> 
       					</td>
       					<td>
       						${p.createuser}
       					</td>
       					...
       					<td class="last">
       						<c:choose>
       							<c:when test="${not empty list }">
				                    ...
				                    <a href="javascript:parent.sales('${p.nik}');;parent.closeAction();lewat();" title="pilih">
				                      <img src="${path }/static/decorator/main/pilu/images/icons/expanded.png" alt="pilih" />
				                    </a>
       							</c:when>
       							<c:otherwise>
       								<a href="${path}/master/karyawan/edit/${p.id}/${groupmenuid}" title="edit">
					                      <img src="${path }/static/decorator/main/pilu/images/icons/application_edit.png" alt="Edit" />
					                </a>
					                <a href="${path}/master/karyawan/view/${p.id}/${groupmenuid}" title="view">
					                    <img src="${path }/static/decorator/main/pilu/images/icons/view.png" alt="View" />
					                </a>
					                <a href="javascript:if(confirm('Are you sure want to Delete \'Id : ${p.id} , Group Name : ${groupmenuName}\' ?'))window.location='${path}/master/karyawan/delete/${p.id}/${groupmenuid}';" title="delete">
	        ...
	        <div class="innerButtom">
		        <div class="actions">
		        	<c:if test="${empty list }">
				        <button class="button" type="button" onclick="window.location='${path}/master/karyawan/new';">
