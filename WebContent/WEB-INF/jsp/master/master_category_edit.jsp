...
<body>
	<h3 class="tit"> Master Kategori &gt; ${category.mode } </h3>
	<c:choose>
   		<c:when test="${messageType eq 'error' or messageType eq 'warning' }">
   			<p class="msg ${messageType}">${message }</p>
   		</c:when>
   		<c:otherwise>
   			<c:if test="${category.mode ne 'VIEW'}">
    			<p class="msg info"><fmt:message key="CompleteForm" /> </p>
			    
		   </c:if>
   		</c:otherwise>
   	</c:choose>
   	<form:form commandName="category" name="formpost" method="POST" action="${path}/master/category/save" cssClass="form" >
   		<div class="columns wat-cf">
   			<div class="group">
	            <div class="fieldWithErrors">
		            <form:label path="id"  cssClass="label" cssErrorClass="label labelError">ID Kategori</form:label>
		            <form:errors path="id" cssClass="error" />
	            </div>
	        	<form:hidden path="id" /><input type="text" class="text_field read" value="${category.id }"  cssErrorClass="text_field inputError"  readonly="readonly" />
            ...
            <div class="group">
				...
				<c:choose>
					<c:when test="${category.mode eq 'VIEW'}"><form:hidden path="nama" /><input type="text" class="text_field read" value="${category.nama }" readonly="readonly" size="60"/></c:when>
        	...
        	<div class="group">
	        	...
	        	<c:choose>
					<c:when test="${category.mode eq 'VIEW'}"><form:hidden path="inisial" /><input type="text" class="text_field read" value="${category.inisial }" readonly="readonly" size="3"/></c:when>
        	...
        	<div class="group navform wat-cf">
        		<c:choose>
					<c:when test="${category.mode eq 'VIEW'}"><form:hidden path="id" />
						<button class="button" type="button" onclick="window.location='${path}/master/category'">
						...
						<c:otherwise>
			                ...
			                <button class="button" type="button" onclick="if(confirm('Are you sure want to cancel?'))window.location='${path}/master/category'">
