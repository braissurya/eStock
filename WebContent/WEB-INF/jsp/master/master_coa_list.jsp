	...
	<form name="form" id="form" method="post">
		...
		<div class="inner">
       		<table class="table">
       			<tr>
       				<th>
       					<script>groupList('id', 'form', '${page}', 'ID COA', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       					<script>groupList('nama', 'form', '${page}', 'Nama', '${sort}', '${sort_type}');</script>
       				</th>
					<th>
						<script>groupList('level', 'form', '${page}', 'Level', '${sort}', '${sort_type}');</script>
					</th>
					<th>
						<script>groupList('post', 'form', '${page}', 'Posting', '${sort}', '${sort_type}');</script>
					</th>
					<th>
						<script>groupList('dk', 'form', '${page}', 'Posisi', '${sort}', '${sort_type}');</script>
					</th>
					<th>
       			...
       			<c:forEach items="${listPaging}" var="p" varStatus="s">
       				<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
       					...
       					<td>
							<c:if test="${p.level ne \"1\"}"><c:forEach begin="0" end="${p.level}">&nbsp;&nbsp;&nbsp;</c:forEach></c:if>
       						${p.nama}
       					</td>
       					<td>
       						${p.level}
       					...
       					<td>
       						<c:choose><c:when test="${p.post eq \"1\"}">Ya</c:when><c:otherwise>Tidak</c:otherwise></c:choose>						
       					</td>
       					<td>
       						<c:choose><c:when test="${p.dk eq \"D\"}">Debet</c:when><c:otherwise>Kredit</c:otherwise></c:choose>
       					</td>
       					<td>
       						${p.createuser}
       					</td>
       					...
       					<td class="last">
							<a href="${path}/master/coa/edit/${p.id}/${groupmenuid}" title="edit">
			                      <img src="${path }/static/decorator/main/pilu/images/icons/application_edit.png" alt="Edit" />
			                </a>
			                <a href="${path}/master/coa/view/${p.id}/${groupmenuid}" title="view">
			                    <img src="${path }/static/decorator/main/pilu/images/icons/view.png" alt="View" />
			                </a>
			                <a href="javascript:if(confirm('Are you sure want to Delete \'Id : ${p.id} , Group Name : ${groupmenuName}\' ?'))window.location='${path}/master/coa/delete/${p.id}/${groupmenuid}';" title="delete">
	        ...
	        <div class="innerButtom">
		        <div class="actions">
			        <button class="button" type="button" onclick="window.location='${path}/master/coa/new';">
