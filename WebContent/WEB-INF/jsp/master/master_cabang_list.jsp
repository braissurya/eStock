	...
	<form name="form" id="form" method="post">
		...
		<div class="inner">
       		<table class="table">
       			<tr>
       				<th>
       					<script>groupList('id', 'form', '${page}', 'ID Cabang', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
					...
					<th>
						<script>groupList('jeniscabang', 'form', '${page}', 'Jenis', '${sort}', '${sort_type}');</script>
					</th>
					<th>
       			...
       			<c:forEach items="${listPaging}" var="p" varStatus="s">
       				<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
       					...
       					<td class="last">
							<a href="${path}/master/cabang/edit/${p.id}/${groupmenuid}" title="edit">
			                      <img src="${path }/static/decorator/main/pilu/images/icons/application_edit.png" alt="Edit" />
			                </a>
			                <a href="${path}/master/cabang/view/${p.id}/${groupmenuid}" title="view">
			                    <img src="${path }/static/decorator/main/pilu/images/icons/view.png" alt="View" />
			                </a>
			                <a href="javascript:if(confirm('Are you sure want to Delete \'Id : ${p.id} , Group Name : ${groupmenuName}\' ?'))window.location='${path}/master/cabang/delete/${p.id}/${groupmenuid}';" title="delete">
	        ...
	        <div class="innerButtom">
		        <div class="actions">
			        <button class="button" type="button" onclick="window.location='${path}/master/cabang/new';">
