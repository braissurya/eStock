...
<body>
	...
	<h1> Master User ${groupmenuName}</h1>
    	...
    	<input type="submit" name="btsearch"  value=" " class="btsearch" title="search">
            ...
            <div class="inner">
      		 	<table class="table">
					<tr>				
						<th>
							<script>groupList('id', 'form', '${page}', 'ID User', '${sort}', '${sort_type}');</script>
						</th>
						<th>
							<script>groupList('namaGroup', 'form', '${page}', 'Group Menu', '${sort}', '${sort_type}');</script>
						</th>
						<th>
							<script>groupList('username', 'form', '${page}', 'Username', '${sort}', '${sort_type}');</script>
						</th>
					...
					<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
						...
						<td>
							<c:choose>
								<c:when test="${p.passwordDecrypt eq \"123bcd\"}">
									default : 123bcd
								</c:when>
								<c:otherwise>
									**********
						...
						<td class="last">
								 <a href="javascript:if(confirm('Are you sure want to Reset Password \'Id : ${p.id} , Group Name : ${groupmenuName}\' ?'))window.location='${path}/admin/user/reset/${p.id}/${groupmenuid}';" title="RESET Password">
			                      <img src="${path }/static/decorator/main/pilu/images/icons/back_undo.png" alt="RESET Password" />
			                   </a>
