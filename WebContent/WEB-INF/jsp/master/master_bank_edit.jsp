...
<body>
	<h3 class="tit"> Master Bank &gt; ${bank.mode } </h3>
	<c:choose>
   		<c:when test="${messageType eq 'error' or messageType eq 'warning' }">
   			<p class="msg ${messageType}">${message }</p>
   		</c:when>
   		<c:otherwise>
   			<c:if test="${bank.mode ne 'VIEW'}">
    			<p class="msg info"><fmt:message key="CompleteForm" /> </p>
			    
		   </c:if>
   		</c:otherwise>
   	</c:choose>
   	<form:form commandName="bank" name="formpost" method="POST" action="${path}/master/bank/save" cssClass="form" >
   		<div class="columns wat-cf">
   			<div class="group">
	        	...
	        	<form:hidden path="id" /><input type="text" class="text_field read" value="${bank.id }"  cssErrorClass="text_field inputError"  readonly="readonly" />
            ...
            <div class="group">
				...
				<c:choose>
					<c:when test="${bank.mode eq 'VIEW'}"><form:hidden path="nama" /><input type="text" class="text_field read" value="${bank.nama }" readonly="readonly" size="60"/></c:when>
            ...
            <div class="group">
            	<div class="fieldWithErrors">
	            	<form:label path="kode_bi" cssClass="label" cssErrorClass="label labelError">Kode BI</form:label>
	                <form:errors path="kode_bi" cssClass="error" />
	        	</div>
				<c:choose>
					<c:when test="${bank.mode eq 'VIEW'}"><form:hidden path="kode_bi" /><input type="text" class="text_field read" value="${bank.kode_bi }" readonly="readonly" size="10"/></c:when>
					<c:otherwise><form:input path="kode_bi"  id="target" cssClass="text_field" cssErrorClass="text_field inputError" size="10" maxlength="10"/></c:otherwise>
		  		</c:choose>
                  <span class="description"></span>
        	</div>
        	...
        	<div class="group navform wat-cf">
        		<c:choose>
					<c:when test="${bank.mode eq 'VIEW'}"><form:hidden path="id" />
