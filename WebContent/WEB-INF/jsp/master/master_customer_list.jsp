	...
	<form name="form" id="form" method="post">
		...
		<div class="inner">
       		<table class="table">
       			<tr>
       				<th>
       					<script>groupList('kode', 'form', '${page}', 'Kode Customer', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       					<script>groupList('nama', 'form', '${page}', 'Nama Customer', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       				...
       				<th>
       					<script>groupList('limit_hutang', 'form', '${page}', 'Limit Hutang', '${sort}', '${sort_type}');</script>
       				</th>
       				<th>
       			...
       			<c:forEach items="${listPaging}" var="p" varStatus="s">
       				<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
       					...
       					<td>
       						<fmt:formatNumber type="currency" currencySymbol="Rp. "  value="${p.limit_hutang}" maxFractionDigits="2" minFractionDigits="0" ></fmt:formatNumber> 
       					</td>
       					<td>
       						<fmt:formatNumber type="currency" currencySymbol="Rp. "  value="${p.total_hutang}" maxFractionDigits="2" minFractionDigits="0" ></fmt:formatNumber> 
       					</td>
       					<td>
       						${p.createuser}
       					</td>
       					...
       					<td class="last">
       						<c:choose>
       							<c:when test="${not empty list }">
       								<a href="javascript:$('#customer\\.kode', top.document).val('${p.kode}');parent.kostumer('${p.kode}');parent.closeAction();lewat();" title="pilih">
				                      <img src="${path }/static/decorator/main/pilu/images/icons/expanded.png" alt="pilih" />
				                    </a>
       							</c:when>
       							<c:otherwise>
									<a href="${path}/master/customer/edit/${p.id}/${groupmenuid}" title="edit">
					                      <img src="${path }/static/decorator/main/pilu/images/icons/application_edit.png" alt="Edit" />
					                </a>
					                <a href="${path}/master/customer/view/${p.id}/${groupmenuid}" title="view">
					                    <img src="${path }/static/decorator/main/pilu/images/icons/view.png" alt="View" />
					                </a>
					                <a href="javascript:if(confirm('Are you sure want to Delete \'Id : ${p.id} , Group Name : ${groupmenuName}\' ?'))window.location='${path}/master/customer/delete/${p.id}/${groupmenuid}';" title="delete">
	        ...
	        <div class="innerButtom">
		        <div class="actions">
		        	<c:if test="${empty list }">
				        <button class="button" type="button" onclick="window.location='${path}/master/customer/new';">
