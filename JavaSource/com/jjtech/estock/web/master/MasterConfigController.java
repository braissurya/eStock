...
import com.jjtech.estock.validator.ConfigValidator;
import com.jjtech.estock.web.ParentController;

/**
 * @author 		: Rudy
 * @since		: Apr 7, 2013 10:25:42 PM
 * @Description : inputan master Config
 */

@Controller
@RequestMapping("/admin/config")
public class MasterConfigController extends ParentController{
	
	@InitBinder
	public void initBinder(WebDataBinder binder) {
		binder.setValidator(new ConfigValidator());
...
//	bisa lebih dari satu model attribute, bisa juga digunakan sebagai reference data
	...
	public String show(Model model,HttpServletRequest request) {	
		logger.debug("Halaman: Master Config, method: SHOW");
		Utils.generatePaging(dbService, request, model, "config", 0);
		return "master/master_config_list";
	}
	
	//input baru
	@RequestMapping(value="/new", method=RequestMethod.GET)
	public String insert(@ModelAttribute("config") Config config) {
		logger.debug("Halaman:  Master Config, method: NEW");
		config.setMode("NEW");
		return "master/master_config_edit";
	}
	...
	public String update(@ModelAttribute("config") Config config, @PathVariable Integer uname, Model model, @PathVariable Integer jenis) { //@PathVariable berarti parameter "uname" langsung di bind ke string "uname"
		logger.debug("Halaman:  Master Config, method: EDIT");
		
		Config tmp=dbService.selectListConfig(uname, jenis).get(0);
		BeanUtils.copyProperties(tmp, config);
		config.setMode("EDIT");
		return "master/master_config_edit";
	}
	...
	public String view(@ModelAttribute("config") Config config, @PathVariable Integer uname, @PathVariable Integer jenis) { //@PathVariable berarti parameter "uname" langsung di bind ke string "uname"
		logger.debug("Halaman:  Master Config, method: View");
	...
	public String save(@Valid @ModelAttribute("config") Config config, BindingResult result, HttpServletRequest request, Model model, RedirectAttributes ra) throws MailException, MessagingException {
		logger.debug("Halaman:  Master Config, method: SAVE");
		...
		String pesan ="";
		try{
			pesan = dbService.saveConfig(config, currentUser);
			//balikin ke layar list input
		}catch (Exception e) {
			e.printStackTrace();
			pesan=messageSource.getMessage("submitfailed", new String[]{"Master Config",""+config.keterangan,""},null);
				email.send(
					true, props.getProperty("email.from"),
					props.getProperty("admin.email.to").split( ";" ), null, null,
					"ERROR pada eStock modul Config", Utils.errorExtract(e), null);
	...
	public String delete(@PathVariable Integer uname, RedirectAttributes ra, HttpServletRequest request, @PathVariable Integer jenis) throws MailException, MessagingException {
		logger.debug("Halaman: Master Config, method: DELETE");
		String pesan ="";
		try{
			//currently logged in user
			User currentUser = (User) request.getSession(false).getAttribute("currentUser");
			Config config=dbService.selectListConfig(uname, jenis).get(0);
			config.setMode("DELETE");
			pesan = dbService.saveConfig(config, currentUser);
			
		}catch (Exception e) {
			e.printStackTrace();
			pesan=messageSource.getMessage("submitfailed", new String[]{"Master Config","","diproses"},null);
				email.send(
					true, props.getProperty("email.from"),
					props.getProperty("admin.email.to").split( ";" ), null, null,
					"ERROR pada eStock modul Config", Utils.errorExtract(e), null);
