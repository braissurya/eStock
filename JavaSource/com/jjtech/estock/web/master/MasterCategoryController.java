...
public class MasterCategoryController extends ParentController{
...
//		map.put("AllUser", dbService.selectDropDown("id", "group_menu_id \"desc\" ,username", "user", "active = 1", "username"));
	...
	public String show(Model model,HttpServletRequest request) {	
		logger.debug("Halaman: Master Category, method: SHOW");
		Utils.generatePaging(dbService, request, model, "category", 0);
		return "master/master_category_list";
	}
	
	//input baru
	@RequestMapping(value="/new", method=RequestMethod.GET)
	public String insert(@ModelAttribute("category") Category category) {
		logger.debug("Halaman:  Master Category, method: NEW");
		category.setMode("NEW");
	...
	public String update(@ModelAttribute("category") Category category, @PathVariable Integer uname, Model model) { //@PathVariable berarti parameter "uname" langsung di bind ke string "uname"
		logger.debug("Halaman:  Master Category, method: EDIT");
		
		Category tmp=dbService.selectListCategory(uname).get(0);
		BeanUtils.copyProperties(tmp, category);
		category.setMode("EDIT");
		return "master/master_category_edit";
	}
	
	//view data
	@RequestMapping(value="/view/{uname}", method=RequestMethod.GET) //mapping request GET saja ke method ini, menerima parameter "uname"
	public String view(@ModelAttribute("category") Category category, @PathVariable Integer uname) { //@PathVariable berarti parameter "uname" langsung di bind ke string "uname"
		logger.debug("Halaman:  Master Category, method: View");
		
		Category tmp=dbService.selectListCategory(uname).get(0);
		BeanUtils.copyProperties(tmp, category);
		category.setMode("VIEW");
		return "master/master_category_edit";
	}
	...
	public String save(@Valid @ModelAttribute("category") Category category, BindingResult result, HttpServletRequest request, Model model, RedirectAttributes ra) throws MailException, MessagingException {
		logger.debug("Halaman:  Master Category, method: SAVE");
		...
		String pesan ="";
		try{
			pesan = dbService.saveCategory(category, currentUser);
			//balikin ke layar list input
		}catch (Exception e) {
			e.printStackTrace();
			pesan=messageSource.getMessage("submitfailed", new String[]{"Master Category",""+category.nama,""},null);
				email.send(
					true, props.getProperty("email.from"),
					props.getProperty("admin.email.to").split( ";" ), null, null,
					"ERROR pada eStock modul Category", Utils.errorExtract(e), null);
	...
	public String delete(@PathVariable Integer uname, RedirectAttributes ra, HttpServletRequest request) throws MailException, MessagingException {
		logger.debug("Halaman: Master Category, method: DELETE");
		String pesan ="";
		try{
			//currently logged in user
			User currentUser = (User) request.getSession(false).getAttribute("currentUser");
			Category category=dbService.selectListCategory(uname).get(0);
			category.setMode("DELETE");
			pesan = dbService.saveCategory(category, currentUser);
			
		}catch (Exception e) {
			e.printStackTrace();
			pesan=messageSource.getMessage("submitfailed", new String[]{"Master Category","","diproses"},null);
				email.send(
					true, props.getProperty("email.from"),
					props.getProperty("admin.email.to").split( ";" ), null, null,
					"ERROR pada eStock modul Category", Utils.errorExtract(e), null);
