 ...
 */
@Controller
@RequestMapping("/tandaterima")
public class InputTandaTerimaController extends ParentController {
	...
	public Map<String, Object> reff(){
		...
		map.put("lsstatus_krm", dbService.selectDropDown("jenis", "keterangan", "lst_config", "active = 1 and id=12", "keterangan"));
		...
		if (request.getParameter("cari")!=null) {
			...
			if(!result.hasErrors()){
				...
				if(!lstrans.isEmpty()){
					...
					if(gudang.trans.posisi_id!=6){
						gudang.trans.no_trans=null;
						errors.rejectValue("trans_no", null, "Posisi Transaksi saat ini masih di "+gudang.trans.posisi_idKet);	
					}else if(gudang.trans.flag_kirim!=1){
						gudang.trans.no_trans=null;
						errors.rejectValue("trans_no", null, "Tidak ada pengiriman untuk transaksi ini");	
					}else{
	...
	public String save(@Valid @ModelAttribute("gudang") Gudang gudang, BindingResult result, HttpServletRequest request, Model model, RedirectAttributes ra) throws MailException, MessagingException {
		logger.debug("Halaman: Tanda Terima, method: SAVE");
		...
		if (!result.hasErrors()) {
			BindException errors = new BindException(result);
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "trans.status_kirim",  "NotEmpty", new String[]{""},null);
			...
			if(!lstrans.isEmpty()){
				...
				if(trans.jenis==4){	
					if(gudang.trans.customer_id!=null)gudang.trans.customer=dbService.selectListCustomer(trans.customer_id).get(0);
				}else if(trans.jenis==3){
					if(gudang.trans.supplier_id!=null)gudang.trans.supplier=dbService.selectListSupplier(trans.supplier_id).get(0);
				}else{
					throw new RuntimeException ("Page not found");
				}

				gudang.trans.lsTransDet=dbService.selectListTransDet(trans.trans_id, null, null, null, null);
				if(trans.sales_id!=null)gudang.trans.karyawan=dbService.selectListKaryawan(trans.sales_id,null).get(0);

				gudang.trans.sub_total_harga=trans.total_harga+trans.total_disc;
				gudang.trans.persen_disc=(trans.total_disc/trans.sub_total_harga)*100;
				gudang.trans.total_harga=trans.total_harga;
				gudang.trans.total_disc=trans.total_disc;
		...
		try{
			...
			pesan = dbService.saveTandaTerima(gudang, currentUser);
		//balikin ke layar list input
		}catch (Exception e) {
			pesan=messageSource.getMessage("submitfailed", new String[]{"Save Tanda Terima","","diproses"},null);
			e.printStackTrace();
			email.send(
		...
		return "redirect:/tandaterima/administration";
