...
import com.jjtech.estock.validator.CabangValidator;
import com.jjtech.estock.web.ParentController;

/**
 * @author 		: Rudy
 * @since		: Apr 7, 2013 10:25:42 PM
 * @Description : inputan master Cabang
 */
...
public class MasterCabangController extends ParentController{
	...
	public void initBinder(WebDataBinder binder) {
		binder.setValidator(new CabangValidator());
...
//	bisa lebih dari satu model attribute, bisa juga digunakan sebagai reference data
	...
	public Map<String, Object> reff(){
		...
		map.put("JenisCabang", dbService.selectDropDown("jenis", "keterangan", "lst_config", "active =1 and id = 4", "keterangan"));
		return map;
	}
	...
	public String show(Model model,HttpServletRequest request) {	
		...
		Utils.generatePaging(dbService, request, model, "cabang", 0);
		return "master/master_cabang_list";
	}
	
	//input baru
	@RequestMapping(value="/new", method=RequestMethod.GET)
	public String insert(@ModelAttribute("cabang") Cabang cabang) {
		logger.debug("Halaman:  Master Cabang, method: NEW");
		cabang.setMode("NEW");
		return "master/master_cabang_edit";
	}
	
	@RequestMapping(value="/edit/{uname}", method=RequestMethod.GET) //mapping request GET saja ke method ini, menerima parameter "uname"
	public String update(@ModelAttribute("cabang") Cabang cabang, @PathVariable Integer uname, Model model) { //@PathVariable berarti parameter "uname" langsung di bind ke string "uname"
		logger.debug("Halaman:  Master Cabang, method: EDIT");
		
		Cabang tmp=dbService.selectListCabang(uname).get(0);
	...
	public String view(@ModelAttribute("cabang") Cabang cabang, @PathVariable Integer uname) { //@PathVariable berarti parameter "uname" langsung di bind ke string "uname"
		logger.debug("Halaman:  Master Cabang, method: View");
	...
	public String save(@Valid @ModelAttribute("cabang") Cabang cabang, BindingResult result, HttpServletRequest request, Model model, RedirectAttributes ra) throws MailException, MessagingException {
		...
		try{
			...
			e.printStackTrace();
			pesan=messageSource.getMessage("submitfailed", new String[]{"Master Cabang",""+cabang.nama,""},null);
				email.send(
					true, props.getProperty("email.from"),
					props.getProperty("admin.email.to").split( ";" ), null, null,
					"ERROR pada eStock modul Cabang", Utils.errorExtract(e), null);
	...
	public String delete(@PathVariable Integer uname, RedirectAttributes ra, HttpServletRequest request) throws MailException, MessagingException {
		logger.debug("Halaman: Master Cabang, method: DELETE");
		String pesan ="";
		try{
			//currently logged in user
			User currentUser = (User) request.getSession(false).getAttribute("currentUser");
			Cabang cabang=dbService.selectListCabang(uname).get(0);
			cabang.setMode("DELETE");
			pesan = dbService.saveCabang(cabang, currentUser);
			
		}catch (Exception e) {
			e.printStackTrace();
			pesan=messageSource.getMessage("submitfailed", new String[]{"Master Cabang","","diproses"},null);
				email.send(
					true, props.getProperty("email.from"),
					props.getProperty("admin.email.to").split( ";" ), null, null,
					"ERROR pada eStock modul Cabang", Utils.errorExtract(e), null);
